...
typedef int *(*sort_func_cb) (int *numbers, int count);
...
int *bubble_sort(int *numbers, int count)
{
		...
		for (i= 0; i < count; i++) {
				for (j = 0; j < count - 1; j++) {
						if (reverse_order(target[j], target[j + 1]) > 0) {
								temp = target[j + 1];
								target[j + 1] = target[j];
								target[j] = temp;
						}
...
void test_sorting(int *numbers, int count, sort_func_cb sort)
{
		int i = 0;
...
int main(int argc, char *argv[])
{
		...
		test_sorting(numbers, count, selection_sort);
